...
let rec type_expr {ptyp_desc; ptyp_loc= loc; _} =
  ...
  | Ptyp_var v ->
      Repr.Var v
  | Ptyp_tuple args ->
      Repr.Tuple (List.map type_expr args)
  | Ptyp_constr (t, args) -> (
      ...
      | "list", [arg] ->
          Repr.List (type_expr arg)
      | "array", [arg] ->
          Repr.Array (type_expr arg)
      | ctor, args ->
          Repr.(App (Custom ctor, List.map type_expr args)) )
  | _ ->
...
module Type_declaration = struct
  let constructor_declaration ~loc {pcd_name; pcd_vars; pcd_args; _} =
    ...
    let variant_args =
      match pcd_args with
      | Pcstr_tuple ts ->
          Repr.Anonymous (List.map (fun t -> type_expr t) ts)
      | Pcstr_record records ->
          Repr.Labeled
            (List.map
               (fun r -> (r.pld_name.txt, type_expr r.pld_type))
               records )
    in
