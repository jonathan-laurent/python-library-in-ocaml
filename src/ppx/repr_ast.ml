...
let atomic_type_ast ~loc = function
  | Bool ->
      [%expr Python_libgen.Repr.Bool]
  | Int ->
      [%expr Python_libgen.Repr.Int]
  | Float ->
      [%expr Python_libgen.Repr.Float]
  | String ->
      [%expr Python_libgen.Repr.String]
  | Unit ->
      [%expr Python_libgen.Repr.Unit]
  | Custom s ->
      [%expr Python_libgen.Repr.Custom [%e estring ~loc s]]
  ...
  | Tuple args ->
      [%expr
        Python_libgen.Repr.Tuple
          [%e elist ~loc (List.map (type_expr_ast ~loc) args)]]
  | List arg ->
      [%expr Python_libgen.Repr.List [%e type_expr_ast ~loc arg]]
  | Array arg ->
      [%expr Python_libgen.Repr.Array [%e type_expr_ast ~loc arg]]
  | Option arg ->
      [%expr Python_libgen.Repr.Option [%e type_expr_ast ~loc arg]]
...
let type_definition_ast ~loc = function
  | Alias t ->
      [%expr Python_libgen.Repr.Alias [%e type_expr_ast ~loc t]]
  | Record fields ->
      let field (l, t) =
        [%expr [%e estring ~loc l], [%e type_expr_ast ~loc t]]
      in
...
let value_signature_ast ~loc = function
  | Constant t ->
      [%expr Python_libgen.Repr.Constant [%e type_expr_ast ~loc t]]
  | Function {args; ret} ->
