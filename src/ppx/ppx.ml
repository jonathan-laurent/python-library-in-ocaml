...
module Renaming = struct
  ...
  let rec rename_type_expr = function
    ...
    | Tuple ts ->
        Tuple (List.map ~f:rename_type_expr ts)
    | List t ->
        List (rename_type_expr t)
    | Array t ->
        Array (rename_type_expr t)
    | Option t ->
        Option (rename_type_expr t)

  let rename_variant_args = function
    | Anonymous ts ->
        Anonymous (List.map ~f:rename_type_expr ts)
    | Labeled fs ->
    ...
    | Variant vs ->
        Variant
          (List.map
             ...
             vs )
    | Alias t ->
        Alias (rename_type_expr t)
    | Enum vs ->
        Enum (List.map ~f:convert_constructor_name vs)
