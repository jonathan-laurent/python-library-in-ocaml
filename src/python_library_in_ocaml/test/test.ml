...
let%expect_test "enum" =
  printf !"%{sexp: python_type_declaration}" python_typedef_for_enum ;
  [%expect
    {|
    ((type_name Enum)
     (definition
...
let%expect_test "simple_alias" =
  printf !"%{sexp: python_type_declaration}" python_typedef_for_simple_alias ;
  [%expect
    {|
...
let%expect_test "sum" =
  printf !"%{sexp: python_type_declaration}" python_typedef_for_sum ;
  [%expect
    {|
    ((type_name Sum)
     (definition
...
let%expect_test "with lists" =
  printf !"%{sexp: python_type_declaration}" python_typedef_for_with_lists ;
  [%expect
    {|
    ((type_name WithLists)
     (definition
...
let%expect_test "record" =
  printf !"%{sexp: python_type_declaration}" python_typedef_for_record ;
  [%expect
    {|
    ((type_name Record)
     (definition
