...
type item =
  ...
  | Declare_dataclass of
      {name: string; vars: string list; fields: (string * type_expr) list}
...
let rec show_type ~quote t =
  ...
  | App (ctor, ts) ->
      quote_opt ~quote
        ...
        ^ "]" )
  | Tuple ts ->
      "tuple[" ^ concat ", " (List.map (show_type ~quote) ts) ^ "]"
  | List t | Array t ->
      "list[" ^ show_type ~quote t ^ "]"
  | Option t ->
      show_type ~quote t ^ " | None"
...
let rec show_lvalue = function
  ...
  | Var s ->
      s
  | Field (l, f) ->
      fmt "%s.%s" (show_lvalue l) f
  | Index (l, i) ->
      fmt "%s[%d]" (show_lvalue l) i
...
let rec show_expr = function
  ...
  | Lambda body ->
      fmt "(lambda %s: %s)" arg_var (show_expr body)
  | Tuple [] ->
      "()"
  | Tuple [arg] ->
      fmt "(%s,)" (show_expr arg)
...
let show_instr = function
  | Assign (s, e) ->
      [fmt "%s = %s" s (show_expr e)]
  | Return e ->
      [fmt "return %s" (show_expr e)]
...
let generic = function
  | [] ->
      []
  | vars ->
      [fmt "Generic[%s]" (concat ", " vars)]

let show_alias_def = function
  | Simple t ->
      show_type ~quote:true t
  | Union ts ->
      concat " | " (List.map (show_type ~quote:true) ts)
  | Tagged_union ctors ->
      let aux (tag, ts) =
